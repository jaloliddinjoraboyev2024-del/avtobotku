...
# Tanlangan chatlardagi NFT sotuv xabarlarini Saved Messages'ga forward qilish.
AUTO_NFT_MONITOR = _get_str("AUTO_NFT_MONITOR", default="false").lower() in {"1", "true", "yes", "on"}
NFT_MONITOR_KEYWORDS = tuple(
    keyword.strip().lower()
    for keyword in _get_str(
        "NFT_MONITOR_KEYWORDS",
        default="nft sotiladi,bio nft sotiladi,nft sale,nft narx",
    ).split(",")
    if keyword.strip()
)
